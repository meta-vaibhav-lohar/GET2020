...
public class PriorityQueueImplementation implements PriorityQueue {
	...
	*/
	void maxHeapify(int position) {
		...
		if ((jobsQueue[position].getPriority() < jobsQueue[returnLeftChild(position)]
				.getPriority())
				|| (jobsQueue[position].getPriority() < jobsQueue[returnRightChild(position)]
						.getPriority())) {
			if (jobsQueue[returnLeftChild(position)].getPriority() > jobsQueue[returnRightChild(position)]
					.getPriority()) {
	...
	/**
	* Overriding the enqueue method of the PriorityQueue interface to perform
	* insertion based on the priority
	* 
	...
	*/
	...
	public void enqueue(Job job) {
		if (!isFull()) {
			...
			while ((current != 1)
					&& (jobsQueue[current].getPriority() > jobsQueue[returnParent(current)]
							.getPriority())) {
	...
	/**
	* Overriding the dequeue method of the PriorityQueue interface to perform
	* deletion in the queue based on the priority
	* 
	...
	*/
	public void printQueue() {
		for (int i = 1; i <= queueSize; i++)
			System.out.println(jobsQueue[i].getJobName() + " : "
					+ jobsQueue[i].getPriority());
	}

	/**
	 * Overriding the poll method to poll an element from the queue
	 * Polls means extracting the maximum priority element from the queue
	 * @return {Job} maximum priority job
	 */
	@Override
	public Job poll() {
		Job polledJob = jobsQueue[1];
		jobsQueue[1] = jobsQueue[queueSize--];
		maxHeapify(1);
		return polledJob;
	}
	...
	/**
	 * Overriding the poll method to peek an element from the queue
